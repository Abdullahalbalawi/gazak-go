...
import { DEMO_MODE, createDemoOrder } from "@/lib/demoMode";
...
export default function Checkout() {
  ...
  const [phone, setPhone] = useState(user?.phone || ""), [name, setName] = useState(user?.full_name || ""), [address, setAddress] = useState("");
  const [latitude, setLatitude] = useState(null), [longitude, setLongitude] = useState(null), [paymentMethod, setPaymentMethod] = useState("CASH");
  const [submitting, setSubmitting] = useState(false), [error, setError] = useState(""); const total = subtotal + DELIVERY_FEE;
  ...
  const submit = async (e) => {
    ...
    setSubmitting(true);
    try {
      if (DEMO_MODE) { const order = createDemoOrder({ name: name.trim(), phone: phone.trim(), address: address.trim(), latitude, longitude, paymentMethod, items, deliveryFee: DELIVERY_FEE }); clearCart(); navigate(`/order-success/${order.id}`); return; }
      const { error: pe } = await supabase.rpc("update_my_profile", { p_full_name: name.trim(), p_phone: phone.trim() }); if (pe) throw pe;
      const { data: orderId, error: oe } = await supabase.rpc("create_order", { p_customer_name: name.trim(), p_customer_phone: phone.trim(), p_address: address.trim(), p_payment_method: paymentMethod, p_items: items.map((i) => ({ product_id: i.product_id, quantity: i.quantity })), p_delivery_fee: DELIVERY_FEE, p_latitude: latitude, p_longitude: longitude });
      if (oe) throw oe; if (!orderId) throw new Error("لم يتم إنشاء رقم الطلب"); clearCart(); navigate(`/order-success/${orderId}`);
    } catch (err) { setError(err?.message || "فشل إنشاء الطلب"); } finally { setSubmitting(false); }
  };
  ...
  return <div className="min-h-screen bg-gray-50 pb-24"><CustomerHeader/><div className="max-w-md mx-auto px-4 py-4"><div className="flex items-center gap-2 mb-4"><Link to="/cart"><ArrowLeft className="w-5 h-5 rotate-180"/></Link><h2 className="font-bold text-lg">إتمام الطلب</h2></div>{DEMO_MODE&&<div className="mb-4 rounded-xl bg-amber-100 text-amber-900 p-3 text-xs">الوضع التجريبي: الطلب يُحفظ محلياً ولا يتم تنفيذ دفع حقيقي.</div>}<form onSubmit={submit} className="space-y-5">
    <div className="bg-white rounded-2xl border p-4 space-y-3"><h3 className="font-semibold text-sm">بيانات التواصل</h3><div><Label htmlFor="checkout-phone">رقم الجوال</Label><Input id="checkout-phone" type="tel" value={phone} onChange={(e)=>setPhone(e.target.value)} required/></div><div><Label htmlFor="checkout-name">الاسم</Label><Input id="checkout-name" value={name} onChange={(e)=>setName(e.target.value)} required/></div></div>
