...
declare
 v_uid uuid:=coalesce(p_actor_id,(select auth.uid())); v_order_id uuid; v_subtotal numeric:=0; v_delivery_fee numeric:=15; v_total numeric:=0;
 ...
 v_waiting_stock boolean:=false; v_status public.order_status; v_route_key text;
begin
 ...
 for v_item in select value from jsonb_array_elements(p_items) loop
   ...
   if v_stock<v_quantity then v_waiting_stock:=true; end if;
   v_subtotal:=v_subtotal+(v_price*v_quantity);
 end loop;
 v_total:=v_subtotal+v_delivery_fee;
 v_status:=case when v_waiting_stock then 'WAITING_STOCK'::public.order_status else 'NEW'::public.order_status end;
 v_route_key:=case when p_latitude is not null and p_longitude is not null then floor(p_latitude*100)::text||':'||floor(p_longitude*100)::text else null end;
 insert into public.orders(customer_id,status,payment_status,subtotal,delivery_fee,total,delivery_address,latitude,longitude,metadata,route_key)
 values(v_uid,v_status,'PENDING'::public.payment_status,v_subtotal,v_delivery_fee,v_total,jsonb_build_object('address',coalesce(p_address,'')),p_latitude,p_longitude,
 jsonb_build_object('customer_name',coalesce(p_customer_name,''),'customer_phone',coalesce(p_customer_phone,''),'payment_method',coalesce(p_payment_method,'CASH')),v_route_key)
 ...
 insert into public.order_history(order_id,from_status,to_status,changed_by,note,metadata)
 values(v_order_id,null,v_status,v_uid,case when v_waiting_stock then 'Waiting for stock' else 'Order created' end,jsonb_build_object('stock_waiting',v_waiting_stock));
 insert into public.notifications(user_id,order_id,type,title,message,data)
 values(v_uid,v_order_id,case when v_waiting_stock then 'INVENTORY'::public.notification_type else 'ORDER'::public.notification_type end,
 case when v_waiting_stock then 'الطلب بانتظار توفر المخزون' else 'تم استلام طلبك' end,
 case when v_waiting_stock then 'بعض المنتجات غير متوفرة حالياً. سيتم استكمال الطلب تلقائياً عند توفرها.' else 'وصل طلبك إلينا وجاري معالجته.' end,
 jsonb_build_object('status',v_status,'stock_waiting',v_waiting_stock));
 return jsonb_build_object('order',jsonb_build_object('id',v_order_id,'customer_id',v_uid,'status',v_status,'subtotal',v_subtotal,'delivery_fee',v_delivery_fee,'total',v_total));
end $$;

create or replace function public.manage_inventory(
 p_action text,p_product_id uuid,p_quantity integer,p_notes text default null,
 p_target_party_id uuid default null,p_target_party_type public.custody_party_type default null,p_actor_id uuid default null
) returns jsonb language plpgsql security definer set search_path='' as $$
...
create or replace function public.update_order_status(
 p_order_id uuid,p_action text,p_extra jsonb default '{}'::jsonb,p_actor_id uuid default null
) returns jsonb language plpgsql security definer set search_path='' as $$
declare
 v_uid uuid:=coalesce(p_actor_id,(select auth.uid())); v_role public.user_role; v_order public.orders%rowtype; v_new_status public.order_status;
 v_driver_id uuid; v_target_status public.order_status; v_effective_action text:=p_action; v_result jsonb;
begin
 if v_uid is null then raise exception 'AUTH_REQUIRED'; end if;
 select role into v_role from public.profiles where id=v_uid and is_active=true;
 ...
 if p_action='delete' then
   ...
   delete from public.cylinder_transactions where order_id=p_order_id;
 ...
 if p_action='manualStatus' then
   ...
   v_effective_action:=case
     ...
     when v_order.status in('NEW','ACCEPTED','PREPARING') and v_target_status='CANCELLED' then 'cancel'
     else null end;
 ...
 elsif v_effective_action='cancel' and v_order.status in('NEW','ACCEPTED','PREPARING') and v_role in('customer','admin') then
