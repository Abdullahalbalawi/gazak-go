-- Final canonical privileged RPC definitions. Browser roles have no EXECUTE; Edge Functions use service_role.
-- This migration also removes the legacy signatures left by the original public RPC API.

drop function if exists public.create_order(jsonb,text,text,text,numeric,numeric,text);
drop function if exists public.update_order_status(uuid,text,jsonb);
drop function if exists public.manage_inventory(text,uuid,integer,text,uuid,public.custody_party_type);
...
declare
  v_uid uuid:=coalesce(p_actor_id,(select auth.uid())); v_role public.user_role; v_order public.orders%rowtype; v_new_status public.order_status; v_driver_id uuid; v_existing_count integer;
begin
  if v_uid is null then raise exception 'AUTH_REQUIRED'; end if;
