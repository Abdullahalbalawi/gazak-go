...
begin
  ...
  if not found or v_customer.role <> 'customer' then
    raise exception 'Only active customers can create orders';
  end if;
  ...
  insert into public.orders (
    ...
    auth.uid(), p_customer_name, p_customer_phone, 'NEW',
    0, p_delivery_fee, p_delivery_fee, p_payment_method,
  ...
  where p.role = 'distributor' and p.active = true;
...
begin
  ...
  if not (
    (v_order.status = 'NEW' and p_new_status in ('ACCEPTED','CANCELLED')) or
    ...
    (v_order.status = 'ARRIVED' and p_new_status in ('DELIVERED','CANCELLED'))
  ...
  if p_new_status = 'ASSIGNED' then
    ...
    where o.driver_id = v_order.driver_id
      and o.id <> v_order.id
  ...
  set status = p_new_status,
      assigned_at = case when p_new_status = 'ASSIGNED' then coalesce(assigned_at, now()) else assigned_at end,
      updated_at = now()
  where id = v_order.id;

  if p_new_status = 'DELIVERED' then
    update public.products p
    set reserved_stock = p.reserved_stock - x.qty,
        sold_stock = p.sold_stock + x.qty,
        updated_at = now()
    from (
      select product_id, sum(quantity)::integer qty
      from public.order_items where order_id = v_order.id group by product_id
    ) x
    ...
    set stock = p.stock + x.qty,
        reserved_stock = p.reserved_stock - x.qty,
        updated_at = now()
    from (
      select product_id, sum(quantity)::integer qty
      from public.order_items where order_id = v_order.id group by product_id
  ...
  select distinct p.id,
    'تحديث الطلب',
    'تم تحديث حالة الطلب إلى ' || p_new_status::text,
    v_order.id,
    'order_status'
  from public.profiles p
  where p.id in (v_order.customer_id, v_order.driver_id, v_order.distributor_id)
